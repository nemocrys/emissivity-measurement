...
class PyrometerLumasense:
    ...
    def _get_floats(self):
        """Read floatingpoint values. Only used for SampleMonoAndRatio function"""
        response = self.serial.readline().decode().strip()
        string_valMono = response[:5]
        string_valRatio = response[5:]
        return float(f"{string_valMono[:-1]}.{string_valMono[-1:]}"), float(f"{string_valRatio[:-1]}.{string_valRatio[-1:]}")
    ...
    def set_emissivity(self, emissivity):
        ...
        self.serial.write(cmd.encode())
        self._get_ok()
        assert self.emissivity == round(emissivity * 100,4)
    ...
    def sampleMonoAndRatio(self):
        """Read mono and ratio temperature from device.

        Returns:
            float: mono temperature reading, float: ratio temperature reading.
        """
        try:
            cmd = f"{self.device_id}ek\r"
            self.serial.write(cmd.encode())
            valMono, valRatio  = self._get_floats()
        except Exception as e:
            logger.exception(f"Could not sample PyrometerLumasense.")
            valMono = np.nan
            valRatio = np.nan
        return valMono, valRatio
