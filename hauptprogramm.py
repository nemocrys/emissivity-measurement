...
###########################################################################
    global FileOutName, FileOutNameE, FileOutNameEEnd, AutoStop_Pt, AutoStop_Hp, AutoStop_Py, Folder, istStationaer
...
###########################################################################
    global loop, StartConfig, nextTempIn, nEMess, end, istStationaer
    ...
    if (Soll + Area) > float(listTempPt[-1]) and (Soll - Area) < float(listTempPt[-1])  and istStationaer == True:                                # Solange der Istwert außerhalb des Bereiches ist wird StartConfig auf False gesetzt
        ...
        ### Emissionsanpassug
        if nEMess <= 30:                                                                # das soll nur 16 mal durchgeführt werden, da die Rundung nah 10 - 13 Durschgängen keine Änderung mehr bringt
            with open(Folder + '/' + FileOutNameE,"a", encoding="utf-8") as foE:        # Anpassung durchführen
                ...
                foE.write('\n')
                nEMess += 1
            if nEMess == 30:                                                                       # Speichere die endgültigen Emissionsgrade
