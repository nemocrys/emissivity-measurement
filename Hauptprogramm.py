...
def createFiles():
    ...
    with open(os.path.join(path, "measurement_data.csv"), "w", encoding="utf-8") as f: 
        f.write(f"tTarget,tSample_avg,tSample_std,tRatioPyro_avg,tRatioPyro_std,tkwPyro_avg,tKwPyro_std,E_last,EmisSensor\n")
...
def on_close(event):
    sensor.stop_heizung() # stops heating plate
    print("program was closed correnctly!")
    exit()
...
fig.suptitle("Program will halt if plot is closed",fontsize=14, c="red") # Erzeugt eine Gesamt Überschrifft des Graphen
...
plt.title("Emisivity over time", fontsize=12)
plt.xlabel("Time [min]",fontsize=10)
plt.ylabel("Emisivity [%]",fontsize=10)
plt.grid()
    ...
    for EmisSensor in EmisSensorList: # for every EmisSensor (in every temperature):
        ...
        while True: 
            ...
            if isInMessbereich == True: # start measurement if true
                ...
                tSampleListTemp.append(tSample)
            ...
            if programStart + 3600*12 < calcEnd:
                ...
                plt.close() # Beendet das Skript in den der Plot geschlossen wird
