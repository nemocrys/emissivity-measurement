...
import pyrometer # Steuert das KW-Pyrometer
...
# itterates thru diffrent values of k until the temperature matches the tamperature "tSample" with a tolerance of "tolerance"
# returns the found value of k or k=1 if no value was found
def kAnpassung(tSample, tolerance=1):
    for tempk in np.linspace(0.6, 1.6, 50): # Teste k werte von 0.5 bis 1.5
        k = round(tempk,3)
        print(f"k = {k}")
        RatioPyro.set_k(k) #set as new k
        
        # Determin the avg. Temperature (of the Pyrometer) over one second.
        kTempertaurList = []
        for i in range(10):
            kTempertaurTemp = round(RatioPyro.sample(),2)
            kTempertaurList.append(kTempertaurTemp)
            time.sleep(.1)
        kTempertaur = round(np.mean(kTempertaurList),2) # Durschnittstemp
        
        # Determin if k is in the Area of tSample
        if kTempertaur <= (tSample + tolerance) and kTempertaur >= (tSample - tolerance):
            return k
            break
    print("no fitting k was found, k=1 will be returned.")
    return  1.0
...
def createFiles():
    ...
    with open(os.path.join(path, "data.csv"), "w", encoding="utf-8") as f: 
        f.write("tTarget,tHeizplate,tSample,tRatioPyro,tRatioPyroAvg,tKwPyro,E\n")
        
    with open(os.path.join(path, "measurement_data.csv"), "w", encoding="utf-8") as f: 
        f.write(f"k,tTarget,tSample_avg,tSample_std,tRatioPyro_avg,tRatioPyro_std,tkwPyro_avg,tKwPyro_std,E_last,EmisSensor\n")
...
print(f"\nRatioPyro.k = {RatioPyro.k}\n")

### Prepare Pyro2
pyrometer.logging_on(False)
for name, data in config['Pyrometer_KW'].items():
    KwPyro = pyrometer.PyrometerKW(name, **data)               # Erzeugt einen Eintrag in das Dictionarie
KwPyro.anpassung(100,100)
print(KwPyro.name)

### Variables
e_start = KwPyro.get_pyro_para("e")
e_old = e_start
e_next = e_start
...
ax1 = plt.subplot(211)
# ax1.set_ylim([float(tTargetList[0])-10, float(tTargetList[-1])+10])
ax1.set_ylim([20, float(tTargetList[-1])+20])
tSampleLine,       = ax1.plot(x, tSampleList, label='pt100 Probe',c="black")
tRatioPyroLine,    = ax1.plot(x, tRatioPyroList,c="red",alpha=0.4)
tRatioPyroAvgLine, = ax1.plot(x, tRatioPyroAvgList, label=f"{RatioPyro.name} (Avg)",c="red")
tKwPyroLine,       = ax1.plot(x, tKwPyroList, label=f"{KwPyro.name}",c="blue")
...
for i in range(len(tTargetList)):
    ...
    for EmisSensor in EmisSensorList: # for every EmisSensor (in every temperature):
        ...
        e_old = e_start
        e_next = e_start
        KwPyro.write_pyro_para('e', e_start)
        ...
        while True:
            ...
            tRatioPyro = round(RatioPyro.sample(),2)
            tKwPyro = round(KwPyro.read_pyro(),2)
            tHeizplate = round(sensor.get_istwert(),2)
            E = KwPyro.get_pyro_para("e")

            ### Save data internaly (for plots, stationäermessung)
            tSampleList.append(tSample)
            tRatioPyroList.append(tRatioPyro)
            tRatioPyroAvgList = rollingAvg(tRatioPyroList,8)
            tKwPyroList.append(tKwPyro)
            ...
            with open(os.path.join(path,"data.csv"), "a") as f:
                f.write(f"{tTarget},{tHeizplate},{tSample},{tRatioPyro},{tRatioPyroAvgList[-1]},{tKwPyro},{E}\n")
            ...
            if isInMessbereich == True:
                ...
                isStationaer = stationaerPruefung(tRatioPyroAvgList, tStationaer, tStationaerTolerace)
                if tRatioPyro <= (tTarget + tTolerance) and tRatioPyro >= (tTarget - tTolerance) and isStationaer == True:
                    if kAnpassungDone == False:
                        k = kAnpassung(tSample)
                        kAnpassungDone = True
                ...
                if tHeizplate <= (tTarget + tTolerance) and tSample >= (tTarget - tTolerance) and isStationaer == True:
                    if kAnpassungDone == False:
                        k = kAnpassung(tSample)
                        kAnpassungDone = True
                    isInMessbereich = True
            ...
            if isInMessbereich == True: # start measurement if true
                #print("Emissionsmessung!")
                tSampleListTemp.append(tSample) # Speichert alle Spannungen (in mV) der aktuellen Messung
                tRatioPyroListTemp.append(tRatioPyro) # Speichert alle Temperaturn der aktuellen Messung
                tKwPyroListTemp.append(tKwPyro)
                EListTemp.append(E)
                
                if EmisSensor =="pyro":
                    e_old, e_next = Emissions_Anpassung(tKwPyro, tRatioPyroAvgList[-1], e_old, e_next, 100, 5)
                elif EmisSensor =="pt":
                    e_old, e_next = Emissions_Anpassung(tKwPyro, tSample, e_old, e_next, 100, 5)
                
                KwPyro.write_pyro_para('e', e_old)
                ...
                if data_points >= tTime * 30:
                    ...
                    with open(os.path.join(path,"measurement_data.csv"), "a", encoding="utf-8") as f:
                        line = f"{k},{tTarget},{round(np.mean(tSampleListTemp),2)},{round(np.std(tSampleListTemp),2)},{round(np.mean(tRatioPyroListTemp),2)},{round(np.std(tRatioPyroListTemp),2)},{round(np.mean(tKwPyroListTemp),2)},{round(np.std(tKwPyroListTemp),2)},{E},{EmisSensor}\n"
                        f.write(line)
            ...
            plotData(ax1, x, tRatioPyroList,    tRatioPyroLine)
            plotData(ax1, x, tRatioPyroAvgList, tRatioPyroAvgLine)
            plotData(ax1, x, tKwPyroList,       tKwPyroLine)
